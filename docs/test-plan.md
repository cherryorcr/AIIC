...
| 性能测试 | 规则路径延迟基线、模型路径 p50/p95、并发容量 | 进程内压测 + `model_invocations` 遥测 + locust/k6，见 `docs/performance-test-plan.md` |
...
- 群面：模拟队友反应、无用户发言时自主推进并持久化、暂停语义；
- 匹配评分快照：缓存命中、首写不可变、档案变更失效、个性化题目溯源、中文/英文评分包装兼容；
- 文档解析：fallback 分段映射、模型输出修复归位、上传-解析-确认-属主全流程、类型/大小拒绝。

**最近一次执行结果（2026-08-30，Linux/Python 3.10）**：`40 passed in 6.19s`，无失败。回归执行方式：
...
| T-18 | SSE 事件流（当前为占位实现） | 连接建立并收到 ready 事件；完整流式推送为 planned |
| T-19 | 群面用户不发言 | `/group/advance` 生成队友发言并持久化，同一队友不连续发言 |
| T-20 | 匹配评分重复请求 | 相同档案+JD 命中快照，不重复调用付费模型；快照首写后不可变 |
| T-21 | 网络超时后重放同一回答 | 返回已持久化 Turn（幂等），不重复评分 |
